    ...
    <section id="vehicules" className="relative z-20" data-section="fleet-services">
      ...
      <div className="pt-0 pb-6 bg-white overflow-hidden border-t border-gray-200/80">
        <div className="relative flex overflow-hidden mask-linear-fade mb-6 pt-12 sm:pt-14">
          <div className="flex items-stretch gap-6 animate-scroll-marquee-reverse px-6" style={{ width: "max-content" }}>
            {[...vehiclesToShow, ...vehiclesToShow].map((v, i) => (
              <article
                key={`location-${i}`}
                className="flex-shrink-0 w-[280px] sm:w-[320px] rounded-2xl overflow-hidden border border-primary/30 bg-white shadow-xl shadow-primary/15 hover:shadow-2xl hover:shadow-primary/25 hover:scale-[1.02] hover:border-primary/50 transition-all duration-300"
              >
        ...
        <div className="px-6 py-8">
          {/* Grand titre décoratif en dégradé qui prend tout l'espace */}
          <h2 className="text-[80px] sm:text-[120px] lg:text-[160px] font-extrabold font-display leading-none bg-gradient-to-r from-[#99ffcc] via-[#0d9488] to-[#99ffcc] bg-clip-text text-transparent text-center">
            À LOUER
          </h2>
          <p className="text-[#1E1E1E]/40 text-xs sm:text-sm text-center mt-4 font-light italic">
            Location de véhicules de prestige en conduite autonome sur la Côte d&apos;Azur
          </p>
        </div>
      </div>
    </section>
